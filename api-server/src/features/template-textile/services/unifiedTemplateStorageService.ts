...
class UnifiedTemplateStorageService {
  ...
  async deleteTemplate(templateId: string, request: Request): Promise<void> {
    ...
    // Use the template's own storageUrl to determine storage mode,
    // NOT modeDetectionService — the template might have been saved in
    // a different mode than the current system mode.
    let metadata: TemplateMetadata | null = null;
    let storageMode: string = 'fallback';
    ...
    if (metadata) {
      try {
        if (storageMode === 'full') {
          ...
          // Delete any associated resource files (images, etc.)
