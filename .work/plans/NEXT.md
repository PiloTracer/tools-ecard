...
> **Older session history moved.** Session-status history and the earlier cross-framework / latest-action sections (44 lines) now live in [`NEXT.archive.md`](NEXT.archive.md) (Context budget: history is moved, never deleted). This file keeps the current status, open owner actions, unknowns, and the latest session.
